...
{
    public partial class ExportToFamilyForIFC : System.Windows.Forms.Form
    {
        ...
        public ExportToFamilyForIFC(Document doc, ExternalCommandData commandData)
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (Transaction t = new Transaction(Doc, "Create Sheet For IFC Family"))
            {
                ...
                List<string> ifcExportAs = new List<string> { "IFC ExportAs" };
                ...
                foreach (Element element in elements)
                {
                    ...
                    try
                    {
                        if (element.LookupParameter("IfcExportAs") != null)
                        {  
                           ifcExportAs.Add(element.LookupParameter("IfcExportAs").AsString());   
                        }
                        else
                        {
                            ifcExportAs.Add("");
                ...
                List<List<string>> GL = new List<List<string>> { familyId, eleCategory, familyName, familyType, ifcExportAs };

                string fullPath = Path.GetFullPath(folD); 
                string savePath = Path.Combine(fullPath, "Exported IFC Data"); 
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            using (Transaction transaction = new Transaction(Doc))
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    string ifcExportAsValue = Convert.ToString(row.Cells["IFC ExportAs"].Value);                 
                    ...
                    try
                    {
                        if (elemenT.LookupParameter("IfcExportAs") != null)
                        {
                            var update = elemenT.LookupParameter("IfcExportAs").Set(ifcExportAsValue);      
                        }
                    }
                    catch { }
                }
                ...
                transaction.Commit();
                TaskDialog.Show("Updated Family Types", " IFC Values of Family Types Updated.");
