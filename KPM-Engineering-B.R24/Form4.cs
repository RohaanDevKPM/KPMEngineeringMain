...
namespace KPMEngineeringB.R
{
    public partial class Form4 : System.Windows.Forms.Form
    {
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            ...
            double defaultSize;
            double.TryParse(cb_ConduitSize.Text, out defaultSize);
            conduitSize = defaultSize / 304.8;
            ...
            foreach (Line selectedline in lineList)
            {
                ...
                if (selectedLayer == categoryName)
                {
                    ...
                    try
                    {
                        ...
                        var cnSize = createConduits.get_Parameter(BuiltInParameter.RBS_CONDUIT_DIAMETER_PARAM).Set(conduitSize);
        ...
        private void cb_Conduit_Type_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var conduitSetting = ConduitSizeSettings.GetConduitSizeSettings(Doc);
            var standardName = Doc.GetElement(ConduitTypeInstance.get_Parameter(BuiltInParameter.CONDUIT_STANDARD_TYPE_PARAM).AsElementId()).Name;
            List<string> conduitSizeList= new List<string>();
            var conduitSizes = conduitSetting
                    .First(a => a.Key.Equals(standardName, StringComparison.InvariantCultureIgnoreCase))
                    .Value;

            foreach (var conduitSize in conduitSizes)
            {
                sizes.Add(Math.Round(((conduitSize.NominalDiameter)*304.8),2));
            }
