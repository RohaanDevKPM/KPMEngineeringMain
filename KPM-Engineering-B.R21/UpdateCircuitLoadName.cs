...
namespace KPMEngineeringB.R
{
    public partial class UpdateCircuitLoadName : System.Windows.Forms.Form
    {
        ...
        private void UpdateCircuitLoadName_Load(object sender, EventArgs e)
        {
            ...
            if(collectEleEq.Count > 0)
            { 
                foreach (Element eleEq in collectEleEq)
                {
                    if (eleEq is FamilyInstance familyInstance)
                    {
                        if ((eleEq as FamilyInstance).MEPModel.GetElectricalSystems() != null && !eleEq.Category.Name.Contains(".dwg"))
                        {
                            var getCkt = (eleEq as FamilyInstance).MEPModel.GetElectricalSystems();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (selectedIndex > 0)
            {
                ...
                Level linkLvl = null;
                var collectLvl = new FilteredElementCollector(selectedElementLink)
